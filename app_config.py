import os
import threading
from dotenv import load_dotenv

load_dotenv()


def _as_bool(value: str, default: bool = True) -> bool:
    if value is None:
        return default
    return value.strip().lower() not in ("0", "false", "no", "off", "")


_flag_lock = threading.Lock()
_runtime_flags = {
    "perplexity": _as_bool(os.environ.get("ENABLE_PERPLEXITY"), True),
    "apify": _as_bool(os.environ.get("ENABLE_APIFY"), True),
    "trailers": _as_bool(os.environ.get("ENABLE_TRAILERS"), True),
    "explanations": _as_bool(os.environ.get("ENABLE_EXPLANATIONS"), True),
}


def is_feature_enabled(name: str) -> bool:
    with _flag_lock:
        return bool(_runtime_flags.get(name, True))


def set_feature_flag(name: str, enabled: bool):
    with _flag_lock:
        _runtime_flags[name] = bool(enabled)


def get_feature_flags() -> dict:
    with _flag_lock:
        return dict(_runtime_flags)


def get_startup_readiness() -> dict:
    return {
        "telegram_bot_token": bool(os.environ.get("TELEGRAM_BOT_TOKEN", "").strip()),
        "perplexity_api_key": bool(os.environ.get("PERPLEXITY_API_KEY", "").strip()),
        "airtable_api_key": bool(os.environ.get("AIRTABLE_API_KEY", "").strip()),
        "airtable_base_id": bool(os.environ.get("AIRTABLE_BASE_ID", "").strip()),
        "supabase_url": bool(os.environ.get("SUPABASE_URL", "").strip()),
        "supabase_api_key": bool(os.environ.get("SUPABASE_API_KEY", "").strip() or os.environ.get("SUPABASE_SERVICE_ROLE_KEY", "").strip()),
        "redis_rest_url": bool(
            os.environ.get("REDIS_REST_URL", "").strip()
            or os.environ.get("UPSTASH_REDIS_REST_URL", "").strip()
        ),
        "redis_rest_token": bool(
            os.environ.get("REDIS_REST_TOKEN", "").strip()
            or os.environ.get("UPSTASH_REDIS_REST_TOKEN", "").strip()
            or os.environ.get("REDIS_API_KEY", "").strip()
        ),
    }
